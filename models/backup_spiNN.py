...
# This is a copy of spiNN before I embedded pw generation inside of the network
...
class BuildModel:
    ...
    def load_data(self):
        ...
        pair_count = (self.length*(self.length-1))/2
        self.all_data_pw = np.zeros((
            len(raw_seqs),
            pair_count,
            self.aa_count,
            self.aa_count),np.bool)
        for i,sample in enumerate(raw_seqs):
            pair_index = 0
            for j,char1 in enumerate(sample):
                for k,char2 in enumerate(sample[j+1:]):
                    self.all_data_pw[i,pair_index,self.characters.index(char1),
                                                    self.characters.index(char2)] = 1
                    pair_index += 1
    ...
    def __init__(self,params = {}):
        ...
        # TODO: wipe the need for LoadData, make it internal (without .params drop either)
        ...
        # save dimensions of original data 
        self.sw_dim,self.pw_dim = self.all_data_sw.shape,self.all_data_pw.shape 
        # number of entries in each input data
        self.full_size = (self.all_data_sw[0].size,self.all_data_pw[0].size) 
